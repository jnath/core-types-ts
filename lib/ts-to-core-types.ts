...
function fromTsTypeNode(node: ts.TypeNode, ctx: Context): NodeType | undefined {
  if (ts.isUnionTypeNode(node))
    ...
    if (children[children.length - 1]?.kind === ts.SyntaxKind.CloseParenToken)
      children.pop();

    if (children.length !== 1 || !ts.isTypeNode(children[0]))
      return ctx.handleError(
        ctx.getUnsupportedError(`Parenthesis type not understood`, node)
      );
    ...
    if (node.typeName.kind === ts.SyntaxKind.QualifiedName)
      ...
      return ctx.handleError(
        ctx.getUnsupportedError(`Qualified reference names not supported`, node)
      );
    ...
    return ctx.handleError(
      ctx.getUnsupportedError("Literal type not understood", node.literal)
    );
