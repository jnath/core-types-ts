...
export function convertCoreTypesToTypeScript(
  ...
  const sourceCode = types
    ...
    .map((tsNode) => generateCode(tsNode))
    .join("\n\n");
...
function tsTypeUnion(ctx: Context, node: OrType): ts.TypeNode {
  return factory.createUnionTypeNode(
    node.or.map((elem) => wrapAnnotations(tsTypeAndOrSchema(ctx, elem), elem))
  );
}

function tsTypeIntersection(ctx: Context, node: AndType): ts.TypeNode {
  return factory.createIntersectionTypeNode(
    node.and.map((elem) => wrapAnnotations(tsTypeAndOrSchema(ctx, elem), elem))
  );
}

function tsTypeAndOrSchema(ctx: Context, node: NodeType): ts.TypeNode {
  if (node.type === "and" || node.type === "or") return tsTypeAndOr(ctx, node);
...
function tsType(ctx: Context, node: NodeType): TsTypeReturn {
  ...
  if (_const)
    ...
    return {
      ...
      node: factory.createUnionTypeNode(
        (_enum as Array<unknown>).map((elem) => tsConstType(ctx, node, elem))
      ),
    };
  ...
  if (node.type === "ref") return { type: "flow-type", node: tsRefType(node) };
