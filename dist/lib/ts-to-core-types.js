...
function convertTypeScriptToCoreTypes(sourceCode, options) {
    ...
    /*setParentNodes */ true);
    const declarations = sourceFile.statements.filter((statement) => ts.isTypeAliasDeclaration(statement) ||
...
function fromTsLiteralNode(node, ctx) {
    if (ts.isLiteralExpression(node)) {
        if (ts.isNumericLiteral(node)) {
            ...
            return ctx.handleError(ctx.getUnsupportedError(`Unsupported enum literal kind=${node.kind}`, node));
        }
    }
    else {
        return ctx.handleError(ctx.getUnsupportedError(`Unsupported enum literal kind=${node.kind}`, node));
...
function fromTsEnumNode(node, ctx) {
    ...
    node.members.forEach((member) => {
        ...
        if (ts.isPropertySignature(member) && member.type) {
            ...
            if (((_b = param.type) === null || _b === void 0 ? void 0 : _b.kind) !== ts.SyntaxKind.StringKeyword &&
                ((_c = param.type) === null || _c === void 0 ? void 0 : _c.kind) !== ts.SyntaxKind.NumberKeyword) {
            ...
            ret.additionalProperties =
                (_d = fromTsTypeNode(member.type, ctx)) !== null && _d !== void 0 ? _d : 
...
function fromTsTypeNode(node, ctx) {
    ...
    if (ts.isUnionTypeNode(node))
        ...
        if (((_a = children[0]) === null || _a === void 0 ? void 0 : _a.kind) === ts.SyntaxKind.OpenParenToken)
            children.shift();
        if (((_b = children[children.length - 1]) === null || _b === void 0 ? void 0 : _b.kind) === ts.SyntaxKind.CloseParenToken)
            children.pop();
...
function fromTsTuple(node, ctx) {
    ...
    const elementTypes = elements.map((node) => { var _a; return (_a = fromTsTypeNode((0, ts_helpers_1.tsStripOptionalType)(node), ctx)) !== null && _a !== void 0 ? _a : anyType; });
