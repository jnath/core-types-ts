...
function tsType(ctx, node) {
    ...
    if (_const)
        ...
        return {
            type: "flow-type",
            node: factory.createUnionTypeNode(_enum.map((elem) => tsConstType(ctx, node, elem))),
        };
...
function tsObjectType(ctx, node) {
    ...
    const createQuestionmark = (required) => required ? undefined : factory.createToken(ts.SyntaxKind.QuestionToken);
    const propertyNodes = [
